...
jobjectArray getRecResult(JNIEnv *env, DetPtr faceDetector, const int &size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        std::string recognize = faceDetector->getFacesRecognized(rect);
        char p[recognize.length() + 1];
        int j;
        for(j = 0; j < recognize.length(); j++)
            p[j] = recognize[j];
        p[j] = '\0';
        g_pJNI_VisionDetRet->setRect(env, jDetRet, rect.left(), rect.top(),
                                     rect.right(), rect.bottom(), env->NewStringUTF(p));
    }
    return jDetRetArray;
}
